...
# -*- coding: iso-8859-1 -*-
"""
civiLDAPManage_IndividualSync
=============================
This script use civiLDAPContact_IndividualSync script to sync CiviCRM contact base on an OpenLDAP server. This one is the 'main' script.
...
__usage__ = """
Usage: civiLDAPManage_IndividualSync.py [-h/-?/--h/--help] [-d] [-s/-v] i[-r/--report] [-f/--force] [-l/--limit LIMIT] PATH
  **-h/-?/--help/--h**
...
def main_parseopts():
    ...
    if (not args and not REPORT_MODE) :
        print("\nErreur: Vous devez passer en argument le chemin absolu vers le script civiLDAPContact_IndividualSync.py.\n")
        print(__usage__)
        sys.exit(1)
    else:
        if REPORT_MODE:
...
def run():
    ...
    CONTACT_TABLE = MYSQL_DB_CIVICRM + '.' + MYSQL_CIVICRM_PREFIX + 'contact'
    # create report 
    if REPORT_MODE:
    ...
    try:
        ...
        sql = sql + ' where (' + CONTACT_TABLE + '.ldap_sync=0 '
        if FORCE_COLLECT_MODE:
            # try to add records with sync_status at 2 (running), from dead sync sessions
            sql = sql + ' OR ' + CONTACT_TABLE + '.ldap_sync=2 '
        sql = sql + ' OR ' + CONTACT_TABLE + '.ldap_sync=3) AND (' + CONTACT_TABLE + ".contact_type = 'Individual'"
        sql = sql + ')'
        countsql = 'select count(distinct(' + CONTACT_TABLE + '.id)) ' + sql
        sql = 'select distinct(' + CONTACT_TABLE + '.id) ' + sql
        if (0 != LIMIT):
    ...
    # launch civiLDAPContact_IndividualSync.py for all theses records
    try:
        cpt=0
        for contact in contacts_list:
            ...
            printer(CHILD_PATH+verbosity+str(contact['id']),2)
