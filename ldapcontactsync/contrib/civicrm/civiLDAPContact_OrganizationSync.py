...
# -*- coding: iso-8859-1 -*-
"""
civiLDAPContact_OrganizationSync
====================
...
__usage__ = """
Usage: civiLDAPContac_OrganizationSync.py [-h/-?/--h/--help] [-d] [-s/-v] ID
  **-h/-?/--help/--h**
...
CONTACT_BASE = 'ou=OrganizationsFromCivi,ou=People,'+LDAP_BASE
...
def run():
    ...
    TABLE_ADDRESS = MYSQL_DB_CIVICRM + '.' + MYSQL_CIVICRM_PREFIX + 'address'
    TABLE_PHONE = MYSQL_DB_CIVICRM + '.' + MYSQL_CIVICRM_PREFIX + 'phone'
    TABLE_OPTION_VALUE = MYSQL_DB_CIVICRM + '.' + MYSQL_CIVICRM_PREFIX + 'option_value'
    TABLE_COUNTRY = MYSQL_DB_CIVICRM + '.' + MYSQL_CIVICRM_PREFIX + 'country'
    TABLE_WEBSITE = MYSQL_DB_CIVICRM + '.' + MYSQL_CIVICRM_PREFIX + 'website'
    TABLE_EMAIL = MYSQL_DB_CIVICRM + '.' + MYSQL_CIVICRM_PREFIX + 'email'
    # get contact to sync
    try:
        ...
        sql = ('select '
          + TABLE_CONTACT + '.id,'
          + TABLE_CONTACT + '.contact_type,'
          + TABLE_CONTACT + '.display_name,'
          + TABLE_CONTACT + '.organization_name,'
          + TABLE_CONTACT + '.is_deleted,'
          + TABLE_CONTACT + '.ldap_sync,'
          + "concat_ws(' $ ',street_address,supplemental_address_1,supplemental_address_2) AS adresse,"
          + TABLE_ADDRESS + '.city,'
          + TABLE_ADDRESS + '.postal_code,'
          + TABLE_COUNTRY + '.name AS country,'
          + TABLE_WEBSITE + '.url'
          + ' from ' + TABLE_CONTACT
          + ' left join ' + TABLE_ADDRESS + ' on ' + TABLE_CONTACT + '.id=' + TABLE_ADDRESS + '.contact_id '
          + ' left join ' + TABLE_PHONE + ' on ' + TABLE_CONTACT + '.id=' + TABLE_PHONE + '.contact_id '
          + ' left join ' + TABLE_OPTION_VALUE + ' on (' 
            + TABLE_CONTACT + '.prefix_id=' + TABLE_OPTION_VALUE + '.value '
            + " and option_group_id='6')"
          + ' left join ' + TABLE_COUNTRY + ' on ' + TABLE_ADDRESS + '.country_id=' + TABLE_COUNTRY + '.id '
          + ' left join ' + TABLE_WEBSITE + ' on ' + TABLE_CONTACT + '.id=' + TABLE_WEBSITE + '.contact_id '
          + ' WHERE ' + TABLE_CONTACT + ".contact_type='Organization' AND " + TABLE_CONTACT + '.id=%s ')
        #print sql 
        ...
        c.execute(' SELECT ' 
          ...
          + TABLE_EMAIL + '.email'
          + ' FROM ' + TABLE_CONTACT + ''
          + ' LEFT JOIN ' + TABLE_EMAIL + ' ON ' + TABLE_CONTACT + '.id=' + TABLE_EMAIL + '.contact_id'
          + ' WHERE ' + TABLE_CONTACT + '.is_deleted =0 '
          + ' AND ' + TABLE_EMAIL + '.is_primary=1 '
          + ' AND ' + TABLE_CONTACT + '.id=%s',(SYNCID,))
        emails = c.fetchall()
        ...
        c.close()
        c=db.cursor(cursorclass = MySQLdb.cursors.DictCursor)
        c.execute('SELECT ' + TABLE_CONTACT + '.id,'
          + '' + TABLE_PHONE + '.phone AS work_phone'
          + ' FROM ' + TABLE_CONTACT +  ''
          + ' LEFT JOIN ' + TABLE_PHONE + ' ON ' + TABLE_CONTACT + '.id=' + TABLE_PHONE +'.contact_id'
          + ' WHERE ' + TABLE_CONTACT + '.is_deleted=0 '
          + ' AND ' + TABLE_PHONE + '.phone_type_id=1 '
          + ' AND ' + TABLE_PHONE + '.phone_type_id IS NOT NULL '
          + ' AND ' + TABLE_CONTACT + '.id=%s',(SYNCID,))
        ...
        c=db.cursor(cursorclass = MySQLdb.cursors.DictCursor)
        c.execute('SELECT ' + TABLE_CONTACT + '.id,'
          + '' + TABLE_PHONE + '.phone AS mobile_phone'
          + ' FROM ' + TABLE_CONTACT + ''
          + ' LEFT JOIN ' + TABLE_PHONE + ' ON ' + TABLE_CONTACT + '.id=' + TABLE_PHONE + '.contact_id'
          + ' WHERE ' + TABLE_CONTACT + '.is_deleted=0'
          + ' AND ' + TABLE_PHONE + '.phone_type_id=2 '
          + ' AND ' + TABLE_PHONE + '.phone_type_id IS NOT NULL'
          + ' AND ' + TABLE_CONTACT + '.id=%s',(SYNCID,))
        ...
        c=db.cursor(cursorclass = MySQLdb.cursors.DictCursor)
        c.execute('SELECT ' + TABLE_CONTACT + '.id,'
          + '' + TABLE_PHONE + '.phone AS fax_phone'
          + ' FROM ' + TABLE_CONTACT + ''
          + ' LEFT JOIN ' + TABLE_PHONE + ' ON ' + TABLE_CONTACT + '.id=' + TABLE_PHONE + '.contact_id'
          + ' WHERE ' + TABLE_CONTACT + '.is_deleted=0'
          + ' AND ' + TABLE_PHONE + '.phone_type_id=3'
          + ' AND ' + TABLE_PHONE + '.phone_type_id IS NOT NULL'
          + ' AND ' + TABLE_CONTACT + '.id=%s',(SYNCID,))
    ...
    try:
        printer('mark civicrm_contact '+ SYNCID+' as ldap_sync 2 -starting',2)
        db.query('BEGIN;')
        db.query('SET @DISABLE_TRIGGERS=1;')
        db.query('UPDATE ' + TABLE_CONTACT + ' set ldap_sync=2 where id="'+SYNCID+'";')
    ...
    for field, val in contact.iteritems():
        ...
        if 'id'==field:
            ...
            ldap_entries[-1]['dn'] = 'o=' +  result + ','+ CONTACT_BASE
   ...
   # handle phone_fax for this contact
    ...
    try:
        ...
        printer('mark civicrm_contact '+ SYNCID+' as ldap_sync 1 -success',2)
        db.query('BEGIN;')
        db.query('SET @DISABLE_TRIGGERS=1;')
        db.query('UPDATE ' + TABLE_CONTACT + ' set ldap_sync=1 where id="'+SYNCID+'";')
        db.query('SET @DISABLE_TRIGGERS=NULL;')
        db.query('COMMIT;')
    except MySQLdb.MySQLError, e:
